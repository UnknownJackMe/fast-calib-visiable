...
public:
  ...
  int dictionaryNameToId(const std::string &name)
  {
    if (name == "DICT_4X4_50")
    ...
    RCLCPP_WARN(node_->get_logger(), "Unknown aruco_dictionary '%s'; falling back to DICT_6X6_250", name.c_str());
    return cv::aruco::DICT_6X6_250;
  }
  ...
  QRDetect(rclcpp::Node::SharedPtr node, Params &params)
  {
    ...
    board_ids_.reserve(params.aruco_ids.size());
    for (const auto id : params.aruco_ids)
    {
      board_ids_.push_back(static_cast<int>(id));
    }
    if (board_ids_.size() != TARGET_NUM_CIRCLES)
    {
      RCLCPP_WARN(node_->get_logger(), "Expected 4 aruco_ids, got %zu; using [1, 2, 4, 3]", board_ids_.size());
      board_ids_ = {1, 2, 4, 3};
    }
  ...
  void detect_qr(cv::Mat &image, pcl::PointCloud<pcl::PointXYZ>::Ptr centers_cloud)
  {
    ...
    std::vector<int> boardIds = board_ids_; // IDs order as explained above
